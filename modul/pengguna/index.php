...
<div class="p-6 animate-slide-fade">
  <div class="bg-white rounded-xl shadow-lg p-6 border-t-4 border-green-600">
    <div class="flex flex-col lg:flex-row items-center justify-between mb-8 animate-scale">
      <div>
        <h2 class="text-3xl font-bold text-green-800 mb-2">Data Pengguna</h2>
        ...
        if(isset($_SESSION['pengguna_sukses'])) { ?>
          <div id="alert-message" class="mb-4 bg-gradient-to-r from-green-50 to-green-100 border-l-4 border-green-500 shadow-md p-4 rounded-lg animate-fade-in-down flex justify-between items-center transform transition-all duration-300 hover:scale-[1.02]" role="alert">
            <div class="flex items-center space-x-3">
              ...
              <p class="font-medium text-green-700"><?php echo $_SESSION['pengguna_sukses']; ?></p>
            </div>
    ...
    <div class="mb-6">
      <div class="relative">
        <input type="text" id="searchInput" class="w-full pl-12 pr-4 py-3 bg-gray-50 rounded-lg border border-gray-300 focus:outline-none focus:border-green-500 focus:ring-1 focus:ring-green-500 transition-all duration-200" placeholder="Cari pengguna...">
        ...
        <table id="penggunaTable" class="min-w-full divide-y divide-gray-200 animate-smooth">
          <thead>
            <tr class="bg-gradient-to-r from-green-50 to-green-100">
              <th class="px-4 py-4 text-xs font-semibold text-left cursor-pointer hover:bg-green-200 transition-colors duration-200" data-column="id_pengguna">
              ...
              <th class="px-4 py-4 text-xs font-semibold text-left cursor-pointer hover:bg-green-200 transition-colors duration-200" data-column="nama_lengkap">
                <div class="flex items-center text-green-800">
                  Nama Lengkap
                  <svg class="w-4 h-4 ml-1.5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m-4 4v12m0 0l4-4m-4 4l-4-4"/>
              ...
              <th class="px-4 py-4 text-xs font-semibold text-left cursor-pointer hover:bg-green-200 transition-colors duration-200" data-column="nama_pengguna">
                <div class="flex items-center text-green-800">
                  Nama Pengguna
                  <svg class="w-4 h-4 ml-1.5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m-4 4v12m0 0l4-4m-4 4l-4-4"/>
              ...
              <th class="px-4 py-4 text-xs font-semibold text-left cursor-pointer hover:bg-green-200 transition-colors duration-200" data-column="peran">
                <div class="flex items-center text-green-800">
                  Peran
                  <svg class="w-4 h-4 ml-1.5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m-4 4v12m0 0l4-4m-4 4l-4-4"/>
  ...
  function renderTable(data = tableData) {
      ...
      paginatedData.forEach((row, index) => {
          ...
          tr.innerHTML = `
              ...
              <td class="px-4 py-4 border-b border-gray-200">${row.nama_lengkap}</td>
              <td class="px-4 py-4 border-b border-gray-200">${row.nama_pengguna}</td>
              <td class="px-4 py-4 border-b border-gray-200">${row.peran}</td>
              <td class="px-4 py-4 border-b border-gray-200">${row.dibuat_pada}</td>
              <td class="px-4 py-4 border-b border-gray-200">
                  <div class="flex justify-center gap-4">
                      <button title="Lihat Detail Pengguna" onclick="detailPengguna(${row.id_pengguna})" class="bg-emerald-50 text-emerald-600 p-2 rounded-lg hover:bg-emerald-100 transition-colors group relative">
                          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                          </svg>
                      ...
                      <button title="Edit Data Pengguna" onclick="editPengguna(${row.id_pengguna})" class="bg-amber-50 text-amber-600 p-2 rounded-lg hover:bg-amber-100 transition-colors group relative">
                          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      ${row.nama_pengguna !== 'admin' ? `
                      <button title="Hapus Data Pengguna" onclick="hapusPengguna(${row.id_pengguna})" class="bg-rose-50 text-rose-600 p-2 rounded-lg hover:bg-rose-100 transition-colors group relative">
                          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
