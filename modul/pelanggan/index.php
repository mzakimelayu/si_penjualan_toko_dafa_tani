...
<div class="p-6 animate-slide-fade">
  <div class="bg-white rounded-xl shadow-lg p-6 border-t-4 border-green-600">
    <div class="flex flex-col lg:flex-row items-center justify-between mb-8 animate-scale">
      <div>
        <h2 class="text-3xl font-bold text-green-800 mb-2">Data Pelanggan</h2>
        ...
        if(isset($_SESSION['pelanggan_sukses'])) { ?>
          <div id="alert-message" class="mb-4 bg-gradient-to-r from-green-50 to-green-100 border-l-4 border-green-500 shadow-md p-4 rounded-lg animate-fade-in-down flex justify-between items-center transform transition-all duration-300 hover:scale-[1.02]" role="alert">
            <div class="flex items-center space-x-3">
              ...
              <p class="font-medium text-green-700"><?php echo $_SESSION['pelanggan_sukses']; ?></p>
            </div>
    ...
    <div class="mb-6">
      <div class="relative">
        <input type="text" id="searchInput" class="w-full pl-12 pr-4 py-3 bg-gray-50 rounded-lg border border-gray-300 focus:outline-none focus:border-green-500 focus:ring-1 focus:ring-green-500 transition-all duration-200" placeholder="Cari pelanggan...">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
      <div class="overflow-x-auto">
        <table id="pelangganTable" class="min-w-full divide-y divide-gray-200 animate-smooth">
          <thead>
            <tr class="bg-gradient-to-r from-green-50 to-green-100">
              <th class="px-4 py-4 text-xs font-semibold text-left cursor-pointer hover:bg-green-200 transition-colors duration-200" data-column="id_pelanggan">
              ...
              <th class="px-4 py-4 text-xs font-semibold text-left cursor-pointer hover:bg-green-200 transition-colors duration-200" data-column="no_hp">
                <div class="flex items-center text-green-800">
                  No HP
                  <svg class="w-4 h-4 ml-1.5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m-4 4v12m0 0l4-4m-4 4l-4-4"/>
              ...
              <th class="px-4 py-4 text-xs font-semibold text-left cursor-pointer hover:bg-green-200 transition-colors duration-200" data-column="alamat">
                <div class="flex items-center text-green-800">
                  Alamat
                  <svg class="w-4 h-4 ml-1.5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m-4 4v12m0 0l4-4m-4 4l-4-4"/>
  ...
  function renderTable(data = tableData) {
      ...
      paginatedData.forEach((row, index) => {
          ...
          tr.innerHTML = `
              ...
              <td class="px-4 py-4 border-b border-gray-200">${row.nama_pelanggan}</td>
              <td class="px-4 py-4 border-b border-gray-200">${row.no_hp}</td>
              <td class="px-4 py-4 border-b border-gray-200">${row.alamat}</td>
              <td class="px-4 py-4 border-b border-gray-200">${row.dibuat_pada}</td>
              <td class="px-4 py-4 border-b border-gray-200">
                  <div class="flex justify-center gap-4">
                      <button title="Lihat Detail Pelanggan" onclick="detailPelanggan(${row.id_pelanggan})" class="bg-emerald-50 text-emerald-600 p-2 rounded-lg hover:bg-emerald-100 transition-colors group relative">
                          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                          </svg>
                      ...
                      <button title="Edit Data Pelanggan" onclick="editPelanggan(${row.id_pelanggan})" class="bg-amber-50 text-amber-600 p-2 rounded-lg hover:bg-amber-100 transition-colors group relative">
                          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <button title="Hapus Data Pelanggan" onclick="hapusPelanggan(${row.id_pelanggan})" class="bg-rose-50 text-rose-600 p-2 rounded-lg hover:bg-rose-100 transition-colors group relative">
                          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
