<?php
    $judul_halaman = "Edit Penjualan Produk";
...
<?php include '../../layout/sidebar.php'; ?>
                    ...
                    <div class="bg-stone-100 p-6 rounded-xl space-y-6 border-2 border-green-100">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div class="relative">
                                ...
                                <input type="text" id="noFaktur" name="no_faktur_penjualan" value="<?php echo $penjualan['no_faktur_penjualan']; ?>" class="mt-1 block w-full rounded-xl border-2 border-green-200 bg-white px-4 py-3 focus:border-green-500 focus:ring-2 focus:ring-green-200 focus:outline-none" readonly required>                            
                            </div>
                            
                            <div class="relative">
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Tanggal Penjualan <span class="text-yellow-500">*</span></label>
                                <input type="datetime-local" id="tanggalPenjualan" name="tanggal" value="<?php echo date('Y-m-d\TH:i', strtotime($penjualan['tanggal'])); ?>" class="mt-1 block w-full rounded-xl border-2 border-green-200 bg-white px-4 py-3 focus:border-green-500 focus:ring-2 focus:ring-green-200 focus:outline-none" required>
                            </div>
                        </div>
                    ...
                    <div id="tabelDinamis" class="overflow-x-auto">
                        <table class="w-full">
                            ...
                            <tbody id="detailProduk" class="divide-y divide-green-100">
                                <?php
                                $query = "SELECT dp.*, p.nama_produk, p.harga_jual 
                                         FROM detail_penjualan dp 
                                         JOIN produk p ON dp.id_produk = p.id_produk 
                                         WHERE dp.id_penjualan = $id_penjualan AND dp.status_dihapus = 0";
                                $result = mysqli_query($koneksi, $query);
                                ...
                                while($row = mysqli_fetch_assoc($result)) {
                                    echo "<tr id='produk-{$row['id_produk']}'>";
                                    echo "<td class='px-4 py-3' style='min-width: 300px;'>";
                                    echo "<div class='relative'>";
                                    echo "<input type='text' class='search-produk w-full rounded-xl border-2 border-green-200 bg-white px-4 py-3 focus:border-green-500 focus:ring-2 focus:ring-green-200 focus:outline-none' value='{$row['nama_produk']}'>";
                                    echo "<input type='hidden' name='produk[{$row['id_produk']}][id_produk]' class='produk-id' value='{$row['id_produk']}'>";
                                    echo "<input type='hidden' name='produk[{$row['id_produk']}][kode_produk]' class='produk-kode' value='{$row['id_produk']}'>";
                                    echo "<input type='hidden' name='produk[{$row['id_produk']}][harga_modal]' class='harga-modal' value='0'>";
                                    echo "</div>";
                                    echo "</td>";
                                    echo "<td class='px-4 py-3'>";
                                    echo "<input type='number' name='produk[{$row['id_produk']}][qty]' class='qty w-20 rounded-xl border-2 border-green-200 bg-white px-4 py-3 focus:border-green-500 focus:ring-2 focus:ring-green-200 focus:outline-none' min='1' value='{$row['jumlah']}' required onchange='hitungSubtotal(this)'>";
                                    echo "</td>";
                                    echo "<td class='px-4 py-3'>";
                                    echo "<input type='number' name='produk[{$row['id_produk']}][harga_satuan]' class='harga w-32 rounded-xl border-2 border-green-200 bg-white px-4 py-3 focus:border-green-500 focus:ring-2 focus:ring-green-200 focus:outline-none' min='0' step='0.01' value='{$row['harga_jual']}' required onchange='hitungSubtotal(this)'>";
                                    echo "</td>";
                                    echo "<td class='px-4 py-3'>";
                                    echo "<span class='subtotal'>Rp " . number_format($row['jumlah'] * $row['harga_jual'], 2, ',', '.') . "</span>";
                                    echo "<input type='hidden' name='produk[{$row['id_produk']}][subtotal]' class='subtotal-input' value='" . ($row['jumlah'] * $row['harga_jual']) . "'>";
                                    echo "</td>";
                                    echo "<td class='px-4 py-3'>";
                                    echo "<button type='button' class='text-red-500 hover:text-red-700' onclick='hapusProduk({$row['id_produk']})'>";
                                    echo "<svg class='w-5 h-5' fill='none' stroke='currentColor' viewBox='0 0 24 24'><path stroke-linecap='round' stroke-linejoin='round' stroke-width='2' d='M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16' /></svg>";
                                    echo "</button>";
                                    echo "</td>";
                                    echo "</tr>";
                                }
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="bg-stone-100 p-6 rounded-xl space-y-4 border-2 border-green-100">
                        <div class="flex justify-between items-center p-4 bg-white rounded-xl border-2 border-green-100">
                            ...
                            <span id="subtotal" class="text-lg font-bold text-green-600">Rp <?php echo number_format($penjualan['total'], 2, ',', '.'); ?></span>
                        </div>
                        <div class="flex items-center gap-4 bg-white p-4 rounded-xl border-2 border-green-100">
                            ...
                            <input type="number" id="diskonPersen" name="diskon" min="0" max="100" class="w-24 rounded-xl border-2 border-green-200 px-3 py-2 focus:border-green-500 focus:ring-2 focus:ring-green-200 focus:outline-none" value="<?php echo $penjualan['diskon']; ?>">
                            <span id="diskonNominal" class="text-gray-700 font-medium">Rp <?php echo number_format($penjualan['diskon'], 2, ',', '.'); ?></span>
                        </div>
                        <div class="flex justify-between items-center p-4 bg-green-50 rounded-xl border-2 border-green-200">
                            ...
                            <span id="totalAkhir" class="text-2xl font-bold text-green-600">Rp <?php echo number_format($penjualan['total'], 2, ',', '.'); ?></span>
                            <input type="hidden" name="total_harga" id="totalBayarInput" value="<?php echo $penjualan['total']; ?>">
                        </div>
                        <div class="flex items-center gap-4 bg-white p-4 rounded-xl border-2 border-green-100">
                            <label class="text-sm font-semibold text-gray-700">Dibayar:</label>
                            <input type="number" id="dibayar" name="dibayar" min="0" step="0.01" class="w-full rounded-xl border-2 border-green-200 px-4 py-2 focus:border-green-500 focus:ring-2 focus:ring-green-200 focus:outline-none" value="<?php echo $penjualan['bayar']; ?>" required>
                        </div>
                        <div class="flex justify-between items-center p-4 bg-yellow-50 rounded-xl border-2 border-yellow-200">
                            ...
                            <span id="kembalian" class="text-xl font-bold text-yellow-500">Rp <?php echo number_format($penjualan['kembalian'], 2, ',', '.'); ?></span>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Mendapatkan nilai produkCounter dari id_produk terbesar yang ada
            const existingRows = document.querySelectorAll('tr[id^="produk-"]');
            let maxId = 0;
