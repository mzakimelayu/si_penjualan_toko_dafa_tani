            ...
            <tbody>
                ...
                if(isset($_GET['tipe_filter'])) {
                    ...
                    if(!empty($_GET['tanggal_awal']) && !empty($_GET['tanggal_akhir'])) {
                        ...
                        $where .= " AND sk.tanggal BETWEEN '$tgl_awal 00:00:00' AND '$tgl_akhir 23:59:59'";
                    }
                    ...
                    if(isset($_GET['status']) && $_GET['status'] !== '') {
                        ...
                        $where .= " AND sk.status_dihapus = '$status'";
                    }

                    $query = "SELECT sk.tanggal, sk.status_dihapus, p.no_faktur_penjualan, pl.nama_pelanggan,
                            pr.nama_produk, sk.jumlah
                            FROM stok_keluar sk
                            LEFT JOIN penjualan p ON sk.id_penjualan = p.id_penjualan
                            LEFT JOIN pelanggan pl ON p.id_pelanggan = pl.id_pelanggan
                            JOIN produk pr ON sk.id_produk = pr.id_produk
                            WHERE $where 
                            ORDER BY sk.id_stok_keluar DESC, sk.tanggal DESC";
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        ...
                        while($row = mysqli_fetch_assoc($result)) {
                            ...
                            if($row['status_dihapus'] == 0) {
                                ...
                                if (!empty($row['no_faktur_penjualan']) && !in_array($row['no_faktur_penjualan'], $faktur_selesai)) {
                                    $faktur_selesai[] = $row['no_faktur_penjualan'];
                                    $total_selesai++;
                                }
                            } else {
                                if (!empty($row['no_faktur_penjualan']) && !in_array($row['no_faktur_penjualan'], $faktur_batal)) {
                                    $faktur_batal[] = $row['no_faktur_penjualan'];
                                    $total_batal++;
                                }
                            }
                            
                            if (!empty($row['no_faktur_penjualan']) && !in_array($row['no_faktur_penjualan'], $faktur_unik)) {
                                $faktur_unik[] = $row['no_faktur_penjualan'];
                                $total_transaksi++;
                            }

                            if (!empty($row['nama_pelanggan']) && !in_array($row['nama_pelanggan'], $pelanggan_unik)) {
